...
test("classic, multimode, video, and node stream clients use capacity retry submitter", () => {
  ...
  assert.match(generation, /import \{ submitAsyncJobWithCapacityRetry \} from "\.\/asyncJobSubmit"/);
  assert.match(nodeApi, /import \{ submitAsyncJobWithCapacityRetry \} from "\.\/asyncJobSubmit"/);
